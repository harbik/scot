...
#[derive()]
pub struct CieDE2000<I = D65,C = CieObs1931>(pub DMatrix<f64>, PhantomData<*const C>, PhantomData<*const I>);
...
//	const double deg180InRad = CIEDE2000::deg2Rad(180.0);
	...
	let g = 0.5 * (
		1.0 - (
			bar_c.powi(7) / (bar_c.powi(7) + POW25TO7)
		).sqrt()
	);
	/* Equation 5 */
	...
	/* Equation 11 */
	let delta_h_prime = 2.0 * c_prime_product.sqrt() *
	...
	/* Equation 17 */
	let r_c = 2.0 * (
		bar_c_prime.powi(7) / (bar_c_prime.powi(7) + POW25TO7)
	).sqrt();
	/* Equation 18 */
	let s_l = 1.0 + (
		(0.015 * (bar_l_prime - 50.0).powi(2)) / 
		(
			20.0 + (bar_l_prime - 50.0).powi(2)
		).sqrt()
	);
	/* Equation 19 */
