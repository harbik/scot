    ...
    */
    pub fn new(start: i32, end: i32, step: S) -> Self {
        Self {
            ...
            }, // this it the non-inclusive end range, so we're adding a 1. The InclusiveRange range has no support for i32!
            step,
        }
    }
...
#[derive(Debug)]
pub enum IterInterpolateType {
    // index target domain, left index interval from domain, and fraction
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.iter_to.next() {
            Some(ut) => {
                let j = self.ito;
                self.ito += 1;
                let from_domain_interval =
                    (Into::<S::UnitValueType>::into(ut).value() - self.start) / self.div;
                ...
                match (i, i as usize, from_domain_interval - i as f64) {
                    ...
                    (_, u, h) if u == self.n && h < 1E-6 => {
                        Some(IterInterpolateType::RangeEnd(j, u))
                    }
