...
impl<const I:usize> SpectralData for CieIllLed<I> {
		...
		<Self::StepType as Step>::UnitValueType: From<<L>::UnitValueType> 
	{
		match I {
			ALL => {
				let data = SMatrix::from_data(ArrayStorage(CIE_LED_ILL_DATA));
				sprague_cols(&self.domain(), &domain, &data)
			}
			i@1..=14 => {
				let data = SVectorSlice::<f64, 81>::from_slice(&CIE_LED_ILL_DATA[i-1]);
				sprague_cols(&self.domain(), &domain, &data)
			}
	...
	fn description(&self) -> Option<String> {
		Some("CIE LED Illuminants".to_string())
	}
}
...
static CIE_LED_ILL_DATA: [[f64;81];9] = [ // 380-780-5nm
